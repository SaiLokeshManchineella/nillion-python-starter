...
async def main():
    ...
    new_secret = nillion.NadaValues(
        {
            "vote_A": nillion.SecretInteger(500),
            "vote_B": nillion.SecretInteger(10),
            "vote_C": nillion.SecretInteger(10),
        }
    )
    ...
    computation_time_secrets = nillion.NadaValues({"vote_B": nillion.SecretInteger(1)})
    computation_time_secrets = nillion.NadaValues({"vote_C": nillion.SecretInteger(1)})
    computation_time_secrets = nillion.NadaValues({"vote_D": nillion.SecretInteger(1)})
...
async def test_main():
  result = await main()
  assert vote_C == {'my_output' : 10}
  assert vote_B == {'my_output' : 10}
  assert vote_D == {'my_output' : 10}
